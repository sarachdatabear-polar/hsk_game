...
- ~~**Deco generation prompts + decor-type manifest rows for the 11 new decos**~~ — done 2026-07-08 (v7 follow-ups round): 10 deco prompts (actual `DECO_IDS` count is 10, not 11) in `GENERATION-PROMPTS-P0-copypaste.md` "v7 street deco batch" + 10 `deco-<id>` planned rows in the manifest. Sprite-draw wiring in `drawTieredDeco` deliberately lands with the first installed deco so it can be sized against real art.
- ~~**Per-id procedural scenes or prompt-priority for the 5 new backdrops**~~ — mooted 2026-07-08: PR #33 installed real art for all five backdrops; the shared gradient fallback now only covers the pre-load flash.
- ~~**`doBuy`/`renderStreet` dead-call cleanup**~~ — done 2026-07-08: removed (street canvas is display:none during shop, `show("street")` re-renders on entry); stale comment in `renderStreet` fixed.
- ~~**Empty "Today's Stock" shelf cosmetic state**~~ — done 2026-07-08: `unownedDailyStock()` helper + muted "all stocked up" row, EN/TH strings (`shop.dailyAllOwned`).
- ~~**Shop-registry test hardening**~~ — done 2026-07-08: soundpack ⊆ `PACKS` + every effect id must change `coinBurst` particle kinds (both verified to catch unwired ids).
- ~~**Art-drop raw PNG pruning policy after install**~~ — done 2026-07-08: `intake_art.py --prune [--yes]` (byte-identical winner = settled target), policy in `art-drop/README.md`; first prune removed 52 leftover paths.
