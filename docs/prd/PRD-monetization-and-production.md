...
- **Amendment — ledger-cursor for purchased coins (approved 2026-07-19; ref. `docs/planning/2026-07-12-coin-purchase-golive.md` §3/§6.4).** Server-granted purchased coins ride the `ledger` table, not the max-never-sum wallet fold, which would otherwise let a webhook-incremented cloud wallet evaporate against an equal-or-larger local wallet. `reconcile()` keeps a `lastLedgerAt` cursor in `nbhsk.sync`, fetches `event_id`-tagged ledger rows newer than the cursor, sums their `delta` into `unseenPurchased`, and folds the wallet as `mergeWallet(local, cloud − unseenPurchased) + unseenPurchased` (subtract-then-add neutralizes the already-counted cloud purchase component before the max; the ≥0 clamp absorbs a spent-down cloud side). The cursor advances **before** the merged keys are written, so a crash lands on the self-healing side (cursor-ahead/wallet-heals-from-cloud), never an unhealable double-credit. **Fresh-cursor exception:** when `lastLedgerAt` is empty (new device or post-wipe reinstall) the fold degrades to a plain `max(local, cloud)` adopt — `unseenPurchased` forced to 0 — so a restored, spent-down wallet cannot float back up by prior purchases (PRD §7.4: consumables never restore). Implemented and unit-tested **dark** (`src/sync.js`, `src/cloud.js`, `src/merge.js`; 103 tests in merge/sync/cloud/purchase-poll); activates only once a real provider is wired (Phase 0, owner-gated).
