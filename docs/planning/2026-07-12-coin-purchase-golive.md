...
- **`pollForCredit` credit-signal scoping — DEFER TO PHASE 2 (needs a schema join key; an additive poll tweak is net-negative, proven 2026-07-12).** The poll detects a credit by "local wallet increased at all," which (a) can mis-toast a concurrent gameplay earning as the purchase delta and (b) on a first purchase where `local ≥ cloud+pack` the fresh-cursor adopt eats the pack. **An early additive attempt was built + reverted** (branch `fix/iap-t5-gate3`, never merged): it had `reconcile` return `ledgerCredit = unseen` and gated the poll on it. A high-effort adversarial review (2026-07-12) confirmed THREE new failure modes it introduced — all because **`unseen` is a fold-internal aggregate, not "this purchase's credit":**
  1. **Degraded-ledger false-negative:** when the ledger sub-fetch degrades (RLS/transient) but the cloud wallet's purchase increment is still adopted by the max-fold, `ledgerCredit=0` while the wallet rises → a real, credited purchase toasts "processing"/failure.
  2. **Fresh-cursor lifetime over-report:** `fetchLedgerSince("")` returns every row since epoch, so `unseen` = the user's *lifetime* purchase total → a 1,000-coin pack toasts "5,000 coins."
  3. **Frozen-cursor multi-row over-report:** a prior degraded cycle leaves an earlier grant's row unseen; the recovering poll sums both → over-reports the delta.
  **Root cause / why not additive:** rejecting concurrent-earn, accepting a degraded-but-real purchase, AND reporting the right delta are three requirements that need **per-purchase identity the client doesn't have** — the webhook stores RC's `event_id` on the ledger row, the client holds `orderId` from `purchase()`, and there is no join key between them. **Phase 2 fix direction:** add the join key (store the client-visible order id on the ledger row, or thread `event_id` back through `purchase()`), so the poll credits the *specific* row with the *right* delta; validate on the closed track (T6). The fresh-cursor adopt *eating* the pack is a related fold-level gate — mitigate by forcing a non-fresh-cursor pre-sync before enabling the purchase button. **Do NOT build early** — unexerciseable until a real provider exists, so unit tests can't catch what the review did.
...
**Un-dark runbook (operational — execute with Jordan's go once Phase 0 keys/products exist; nothing here is code, so it stays out of the tree until then):**

1. **Apply the migration to live Supabase** (`docs/supabase/migrations/2026-07-12-iap-golive.sql`) — adds `ledger.event_id` + unique index, the service-role `wallet_guard` exemption, and `grant_purchase(uuid,integer,text,text,text)`. Apply the same way `schema.sql` was (mgmt-API SQL / SQL editor). Additive + idempotent (`add column if not exists`, `create ... if not exists`, `create or replace`). **Do NOT apply early** — while the provider is mock there are no ledger rows, the client already degrades gracefully on the missing `event_id` column (sync.js C1), and it mutates the backend real users' cloud saves run against.
2. **`grant_purchase` service_role EXECUTE smoke test** — the migration ends with `revoke execute ... from public, anon, authenticated`; confirm the service_role grant survived (else every webhook grant silently 500s forever). With the **service key**, call once with a throwaway event id and assert it returns `'granted'` (and a replay returns `'duplicate'`, not an error):
   `select public.grant_purchase('<test-uid>'::uuid, 1, 'smoke', 'smoke-'||gen_random_uuid()::text, null);`
   Must NOT return `permission denied`. Clean up the test ledger/wallet rows after.
3. **Ledger-read RLS reliability** (gate 4 — operational, no clean client-side fix) — verify the deployed RLS actually grants the client `SELECT` on its **own** `ledger` rows before un-darking. A non-missing-column ledger *read* failure (e.g. RLS-denied) while a purchase increment is already in the cloud wallet can double-credit via the graceful-degradation path (cycle-1 adopts, cursor frozen, cycle-2 re-credits). Confirm a signed-in client can read its ledger rows (browser probe or RLS policy review) — the mitigation is confirming the read path is healthy, not a code change.
4. **Gate 5 — defensive `try/finally` around `iapBuy`'s real-path awaits** (main.js wiring, untested-by-design; add when the live provider lands, NOT before — a mock provider honours the never-reject seam contract so it can't strand `iapPending`). A real RC SDK that *throws* (violating the contract) would leave `iapPending` stuck and lock all IAP buttons for the session. Wrap the awaited real path so `iapPending` always clears AND a re-render + error toast fire on throw. **Render-timing note:** the happy path resets `iapPending` *before* `renderShop()/renderAccount()` so buttons re-render enabled — keep those explicit resets; the `catch` must `toast(t("iap.failed")) + renderShop()` and `finally { iapPending = null; }` is the backstop. Ready-to-apply shape:
   ```js
   iapPending = p.id; btn.disabled = true; btn.textContent = t("iap.pending");
   try {
     const prov = provider();
     const r = await prov.purchase(p.id);
     /* … existing mock + real branches, each still clearing iapPending before its renders … */
   } catch (e) {
     toast(t("iap.failed")); renderShop();
   } finally {
     iapPending = null;   // backstop: a throwing SDK can never strand the buttons
   }
   ```
