...
- **T2. Webhook Edge Function (`supabase/functions/rc-webhook`):** verifies the RC auth header, maps event → `{app_user_id, product_id, event_id, transaction_id}`, idempotent insert into `ledger` (unique on `event_id`; `transaction_id` joins the client purchase) + `wallet.coins += pack.coins` + `entitlements` upsert for Supporter — service_role, per the schema's authority rule. Coin amounts resolve server-side from the catalog.
...
- **`grant_purchase` service_role EXECUTE smoke test:** the migration now explicitly grants the six-argument function to `service_role` after revoking client roles; still smoke-call it after applying the migration.
- **`pollForCredit` credit-signal scoping — RESOLVED 2026-07-15.** The ledger now stores RevenueCat `transaction_id` as `order_id`; reconcile returns per-row `{orderId, delta}` attribution; the poll accepts only the exact order returned by `purchase()`. This closes unrelated-earning false positives and lifetime/multi-row over-reporting.
- **Ledger-read reliability — RESOLVED client-side 2026-07-15; operational smoke remains:** non-migration ledger failures now abort before any local/cloud mutation, preventing the previous adopt-then-recredit double count. Confirm RLS before un-darking anyway.
- **Defensive `try/finally` around `iapBuy` — RESOLVED 2026-07-15:** every provider path now clears `iapPending` and re-renders after throws.
...
1. **Apply the migration to live Supabase** (`docs/supabase/migrations/2026-07-12-iap-golive.sql`) — adds ledger event/order identifiers, unique indexes, the service-role wallet exemption, and `grant_purchase(uuid,integer,text,text,text,text)`.
2. **`grant_purchase` service_role EXECUTE smoke test** — the migration ends with `revoke execute ... from public, anon, authenticated`; confirm the service_role grant survived (else every webhook grant silently 500s forever). With the **service key**, call once with a throwaway event id and assert it returns `'granted'` (and a replay returns `'duplicate'`, not an error):
   `select public.grant_purchase('<test-uid>'::uuid, 1, 'smoke', 'smoke-event-'||gen_random_uuid()::text, 'smoke-order-'||gen_random_uuid()::text, null);`
