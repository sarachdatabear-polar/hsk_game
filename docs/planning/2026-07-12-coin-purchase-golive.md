...
- `reconcile()` additionally fetches `ledger` rows for this user with `created_at > lastLedgerAt` and `reason = 'purchase'`, sums their `delta`, adds that **after** the max fold, then advances the cursor (only on successful push, same transaction-ish ordering as `settleDirty`).
- The webhook writes the grant to `ledger` + increments `wallet.coins`; the incremented cloud wallet keeps single-device users correct even without the cursor, while the cursor keeps multi-device/max-fold users correct. Double-count guard: after adding unseen ledger deltas the client's pushed wallet is ≥ the cloud value the grant produced, so a later `max()` never re-adds it; the cursor never re-reads consumed rows.
