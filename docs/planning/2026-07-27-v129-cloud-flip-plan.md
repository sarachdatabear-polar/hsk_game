...
- **Three screens are still orphaned by the same `show()` rewrite that hid quests:** Street Projects, keepsakes, and the resident collection. Deferred **by design** — they are reachable via the rollback flag (`localStorage.setItem("nbhsk.features.catJourney","false")`), so this is a discoverability bug, not data loss. Worth its own plan before the flag is retired: **do not retire the Street rollback flag until these three have a home in Cat Journey.**
  - **CORRECTION (2026-07-30):** this entry originally read "none holds an unclaimed reward." That was **false for Street Projects.** Both results paths credited `bricksForRound()` with no `CAT_JOURNEY_ENABLED` guard (`main.js` quit + finish paths), while the counter (`#street-bricks`) and the only sink (`advanceLandmark`) sit inside the unreachable `#s-street` — so bricks accrued invisibly every round, and because `bricks` is in `BASE_SYNC_KEYS` with a `mergeBricks` fold they also synced to Supabase. Fixed in `304fdd84` by guarding both earn sites on `!CAT_JOURNEY_ENABLED`. Keepsakes and the resident collection **were** verified clear: keepsake creation is driven only from reachable Street UI, and neighbour move-in is triggered by landmark builds that can never advance while bricks are unspendable. Accumulated bricks are deliberately left in place, so a rollback still finds them.
